...
# Перехват всех исключений для вывода в stderr
...
logger.info("Starting full Burmalda server...")
...
rooms: Dict[str, dict] = {}                # room_id -> {name, messages, counter, typing, creator}
voice_rooms: Dict[str, Set[str]] = {}      # room_id -> set(username)

# Создаём заглушку для общей комнаты (будет перезаписана из БД)
rooms["general"] = {
    "name": "Общий чат",
    "messages": [],
    "counter": 1,
...
async def lifespan(app: FastAPI):
    ...
    async with app.state.pool.acquire() as conn:
        logger.info("Creating tables and applying migrations...")
        try:
            # users table
            await conn.execute("""
            ...
            """)
            logger.info("Tables and migrations finished")
        except Exception as e:
            logger.exception("Error during schema creation")
            raise
        ...
        for r in saved_rooms:
            ...
            if rid not in rooms:
                ...
                voice_rooms[rid] = set()
        logger.info(f"Loaded {len(saved_rooms)} rooms")

        # Загрузка сообщений для каждой комнаты
