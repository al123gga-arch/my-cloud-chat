...
# Глобальный перехват ошибок для вывода в stderr
...
async def lifespan(app: FastAPI):
    ...
    # Даём БД время проснуться (важно для бесплатных инстансов)
    await asyncio.sleep(3)
    # Подключаемся с повторами
    pool = None
...
def dm_room_id(u1, u2): return "dm_" + "_".join(sorted([u1, u2]))
def get_online_count(rid): return sum(1 for u in active_connections.values() if u.get("room") == rid)
def get_online_list(): return list(active_connections.keys())

async def broadcast_to_room(room_id, msg, exclude=None):
    data = json.dumps(msg, ensure_ascii=False)
    dead = []
    for u, info in active_connections.items():
...
async def register(username: str = Form(...), password: str = Form(...)):
    ...
    async with app.state.pool.acquire() as conn:
        ...
        await conn.execute("INSERT INTO users VALUES ($1, $2, $3, NOW())", username, hashed, role)
...
@app.get("/users")
async def get_users(): return {"online": get_online_list(), "count": len(get_online_list())}

@app.get("/rooms")
async def get_rooms():
    return {"rooms": [{"id": rid, "name": rdata["name"], "online": get_online_count(rid)} for rid, rdata in rooms.items() if not rid.startswith("dm_")]}
    ...
    try:
        while True:
            ...
            try:
                ...
                continue
            t = obj.get("type", "")
            cur_room = active_connections[username]["room"]
            ...
            if t == "join_room":
                ...
                if new_room not in rooms and not new_room.startswith("dm_"):
                    ...
                    continue
                if cur_room in voice_rooms and username in voice_rooms[cur_room]:
                ...
                try:
                    async with app.state.pool.acquire() as conn:
                        await conn.execute("INSERT INTO rooms VALUES ($1, $2, $3, NOW())", rid, name, username)
                except Exception as e:
                ...
                try:
                    ...
                    continue
                msg_data = {"id": db_id, "room_id": cur_room, "sender": username, "text": text, "timestamp": datetime.now().isoformat(), "reactions": {}, "edited": False}
                if reply_to:
                ...
                if room:
                    ...
                    if rid.startswith("dm_"):
                        parts = rid[3:].split("_")
                        for u in parts:
                            await send_to_user(u, {"type": "typing", "room_id": rid, "users": typing_list})
                    else:
                        await broadcast_to_room(rid, {"type": "typing", "room_id": rid, "users": typing_list})
                ...
                if rid and rid.startswith("dm_"):
                    parts = rid[3:].split("_")
                    other = parts[0] if parts[1] == username else parts[1]
                    await send_to_user(other, {"type": "messages_read", "room_id": rid, "by": username})
